...
import com.evaluation.testproject.databinding.ConfirmationDialogBinding
...
class HomeFragment : BaseFragment() {
    ...
    override fun onResume() {
        ...
        if (homeVM.gamesResponse?.value == null){
            homeVM.getGamesAPi(homeVM.pageNumber)
        }
    }
    ...
    private val callbackObserver = Observer<MainCallBack> { callBack ->
        when (callBack) {
            MainCallBack.PopulateData -> {
                homeVM.gamesResponse.value?.results?.sortBy { it.released }
                populateAdapter()
                homeVM.loadingPagination = false
            }
        }
    }
